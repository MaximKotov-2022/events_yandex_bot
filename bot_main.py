...
def hi_say_first_message(update, context):
    ...
    context.bot.send_message(
        chat_id=chat.id,
        text=(
             "Привет! Это бот для получения информации о событиях Яндекса.\n"
             "Тут можно узнать о всех мероприятиях, подписаться на "
             "обновления, а также узнать о мероприятих в Москве."
        )
    )
...
def send_message(update, context):
    """Отправка сообщений."""
    text = process_information_parsing(update, context)
    chat = update.effective_chat

    buttons = ReplyKeyboardMarkup([
        ['Подписаться на обновления'],
        ['Все мероприятия', 'Меропрития в Москве'],
    ],
...
def subscribe_updates(update, context):
    ...
    old_data = send_message(update, context)
    time.sleep(RETRY_PERIOD)
    while True:
        if old_data != send_message(update, context):
...
def main():
    ...
    updater.dispatcher.add_handler(MessageHandler(
        Filters.text('Подписаться на обновления'),
        subscribe_updates
        )
    )
    updater.dispatcher.add_handler(MessageHandler(
        Filters.text('Меропрития в Москве'),
        events_city
        )
    )
